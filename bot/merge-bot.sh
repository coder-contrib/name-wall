...
agent_review() {
  ...
  prompt="You are reviewing a pull request to a friendly public \"Wall of Names\" at a Coder workshop. The PR author's GitHub login is \"${author}\". Each contributor adds ONE file names/<their-handle>.json with their name and creative HTML/CSS for how it renders. Each entry renders in its own responsive box (~4:3, width varies ~100–340px) and should fill that box and look good at any size in that range. The JSON MAY also include optional showcase fields rendered as plain text under the art: \"role\" (e.g. job title), \"tagline\" (a short blurb), and \"status\" (one of hiring, seeking, open, freelance, learning), plus optional \"hiringFor\" (a role title) and \"link\" (a contact URL like a LinkedIn/GitHub/site or email). These are people optionally sharing that they are hiring, job-seeking, networking, etc. A normal contact link is fine and expected.

Approve ONLY if ALL of these hold:
- The diff adds/edits exactly one file under names/ and it is valid-looking JSON.
- The displayed name, any html/css, AND the optional role/tagline/status text are not hateful, harassing, sexual, a slur, spammy, or impersonating someone else. role/tagline are short plain text (no markup needed); status, if present, should be a sensible value (hiring/seeking/open/freelance/learning); a contact link to a normal site/profile/email is fine (the app already restricts it to http(s)/email).
